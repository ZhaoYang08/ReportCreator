...
BEGIN_MESSAGE_MAP( CDiagramEditor, CWnd )
	...
	ON_COMMAND( ID_EDIT_PASTE_TO_POSITION, OnEditPasteToPosition )
...
/* ============================================================
	...
	Parameters :	CDC*	-	"CDC" to clear, not used.
		...
		if( m_interactMode == MODE_RESIZING )
		{
			...
			if( obj )
			{
				...
				if( oldRect != newRect )
				{
					...
					int count = 0;
					while( ( otherobj = m_objs->GetAt( count++ ) ) )
					{
						if( otherobj->IsSelected() && otherobj != obj )
						{

							double right = otherobj->GetRight() + offsetright;
							double bottom = otherobj->GetBottom() + offsetbottom;
							double left = otherobj->GetLeft() + offsetleft;
							double top = otherobj->GetTop() + offsettop;

							otherobj->SetRect( left, top, right, bottom );
						}
					}
				}
			}
   ...
   ============================================================*/
{
	...
	if( m_objs && m_interactMode == MODE_MOVING )
	{
		...
		while( ( obj = m_objs->GetAt( count++ ) ) )
		{
			...
			{
				CRect rect = obj->GetRect();
				if( m_snap && m_savedMousePoint != point )
				{
...
void CDiagramEditor::OnEditPasteToPosition()
/* ============================================================
	Function :		CDiagramEditor::OnEditPasteToPosition
	Description :	Handler for the "ID_EDIT_PASTE_TO_POSITION" command
	Access :		Protected

	Return :		void
	Parameters :	none

	Usage :			Pasts the content of the internal clipboard 
					to the current paste-position.

   ============================================================*/
{

	PasteToPosition( m_pastePosition );

}

void CDiagramEditor::PasteToPosition( const CPoint& pos )
{

	UnselectAll();
	GetDiagramEntityContainer()->PasteToPosition( pos );
	RedrawWindow();

}

void CDiagramEditor::SetPastePosition( const CPoint& pos )
{
...
CPoint CDiagramEditor::GetPastePosition() const
{
   ...
   ============================================================*/
{
	...
	if( m_objs )
	{
		...
		if( nID == CMD_COPY || 
			nID == CMD_PROPERTIES ||
			nID == CMD_SELECT_GROUP
			)
			dirty = FALSE;

		m_objs->Snapshot();
		m_objs->SendMessageToObjects( nID, TRUE, NULL, static_cast< CWnd* >( this ), dirty );
		RedrawWindow();
	}
...
void CDiagramEditor::Select( CDiagramEntity* obj, BOOL select )
/* ============================================================
	Function :		CDiagramEditor::Select
   ...
   ============================================================*/
{
	...
	if( obj )
		GetDiagramEntityContainer()->Select( obj, select );
