   ...
   ============================================================*/
{
	...
	DDX_Control(pDX, IDC_EDIT_TOP, m_ctrlTop);
	DDX_Control(pDX, IDC_EDIT_RIGHT, m_ctrlRight);
	DDX_Control(pDX, IDC_EDIT_LEFT, m_ctrlLeft);
	DDX_Control(pDX, IDC_EDIT_HEIGTH, m_ctrlHeight);
	DDX_Control(pDX, IDC_EDIT_GRID_SIZE, m_ctrlGridsize);
	DDX_Control(pDX, IDC_EDIT_BOTTOM, m_ctrlBottom);
	DDX_Control(pDX, IDC_STATIC_MARGIN, m_ctrlMargin);
	DDX_Control(pDX, IDC_STATIC_GRID, m_ctrlGrid);
	DDX_Control(pDX, IDC_STATIC_BG, m_ctrlBg);
	DDX_Check(pDX, IDC_CHECK_GRID_VISIBLE, m_grid);
	DDX_Check(pDX, IDC_CHECK_SNAP, m_snap);
	DDX_Text(pDX, IDC_EDIT_BOTTOM, m_editBottom);
	DDX_Text(pDX, IDC_EDIT_HEIGTH, m_editHeight);
	DDX_Text(pDX, IDC_EDIT_LEFT, m_editLeft);
	DDX_Text(pDX, IDC_EDIT_RIGHT, m_editRight);
	DDX_Text(pDX, IDC_EDIT_TOP, m_editTop);
	DDX_Text(pDX, IDC_EDIT_WIDTH, m_editWidth);
	DDX_Radio(pDX, IDC_RADIO_MARGIN, m_restraint);
	DDX_CBIndex(pDX, IDC_COMBO_MEASUREMENTS, m_measurements);
	DDX_Check(pDX, IDC_CHECK_SHOW_MARGINS, m_margins);
	DDX_Text(pDX, IDC_EDIT_ZOOM_STEP, m_zoom);
	DDV_MinMaxUInt(pDX, m_zoom, 1, 100);
	DDX_Text(pDX, IDC_EDIT_ZOOM_LEVEL, m_zoomlevel);
	DDV_MinMaxUInt(pDX, m_zoomlevel, 5, 400);
	DDX_Text(pDX, IDC_EDIT_GRID_SIZE, m_editGridsize);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CEditorSettingsDialog, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_COLOR_GRID, OnButtonColorGrid)
	ON_BN_CLICKED(IDC_BUTTON_COLOR_MARGIN, OnButtonColorMargin)
	ON_BN_CLICKED(IDC_BUTTON_DEFAULT, OnButtonDefault)
	ON_CBN_SELCHANGE(IDC_COMBO_MEASUREMENTS, OnSelchangeComboMeasurements)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
   ...
   ============================================================*/
{
	...
	if( hdc )
	{
		...
		double diff = static_cast< double > ( GetDeviceCaps( hdc, LOGPIXELSX ) ) / static_cast< double >( screenResolutionX );
